...
Get.Partitions <- function(file = "course_partitionInfo"){
  ...
  output <- list(as.character(module.ids), as.character(group.ids), as.character(user.partition.ids))
  output <- as.data.frame(output)
  # Convert to data frame
  
  colnames(output) <- c("module.id", "group.id", "user.partition.id")
  # Set Column Names
