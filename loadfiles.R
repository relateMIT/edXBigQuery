###################################################################################
# This script contains simpmle functions to load the necessary files for analysis #
###################################################################################
...
Read.Gradesheet <- function(file, directory = data.source){
  # Function to read gradesheet within directory - either given or defaults to the global data.source
  # Input of file - is the file name of the xlsx sheet, ex. "gradesheet" NOT "gradesheet.xlsx"
  file.name <- paste(directory, "\\", file, ".xlsx", sep = "")
  data.sheet <- read.xlsx(file.name, 1)
